...
double infinito = 10e30; //tiempo en que ocurren cosas que sabemos no pueden ocurrir
int atendidos = 0; //al principio no hay nadie ya atendido
int inicio_ocio = 0.0; //marcará el momento en que el servidor empieza a estar ocioso
int acum_cola = 0.0; //acumulador de número de clientes en cola por el tiempo en que están en
...
int tiempo_llegada = reloj + generallegadaInt(tlleg); //en ese tiempo llegará el primer cliente
int tiempo_salida = infinito; //nadie puede salir si nadie ha entrado aun
...
int main(int argc, char** argv) {
    ...
    if(streambuffer==1){
        ...
        oss << path << "../resultados/incremento-" << "FIJO"
    ...
    for(unsigned int i=0;i<MEAN;i++){
        ...
        while (atendidos < total_a_atender) //simularemos hasta que hayamos atendido a cierto
        {
            ...
            {
                tiempo_llegada = reloj + generallegadaInt(tlleg); //determinamos cuando llegará
                                                               //el siguiente cliente
                if (servidor_libre) //si el servidor está ocioso
                {
                    servidor_libre = false; //deja de estarlo
                    tiempo_salida = reloj + generaservicioInt(tserv); //determinamos cuando saldrá
        ...
        float porcent_ocio = ocio*100/(float)reloj; //calculamos el porcentaje de tiempo de ocio del servidor
        float media_encola = acum_cola/(float)reloj; //calculamos el número medio de clientes en cola
...
void initialize(float lleg, float serv, int total){
    ...
    tiempo_llegada = reloj + generallegadaInt(tlleg); //en ese tiempo llegará el primer cliente
